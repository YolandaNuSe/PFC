 ...
 *
 * @version  V0.3
 * @date     Monday February 27 2012 15:00
 *
 *******************************************************************************************************/
...
#define TCON_PRE_DIV1                  (0x0   << 0  ) /* DIV1. Source clock/1.    */
...
/* GPCON[CON7] - Configuration bits for Px.7 (not available for port GPIO1). */
#define GPCON_CON7_MSK                 (0x3   << 14 )

/* GPCON[CON6] - Configuration bits for Px.6. */
...
#define CLK_T2                         (0x1   << 4  )
#define CLK_T2_DIS                     (0x0   << 4  ) /* DIS. Dsable T2 clocks.   */
...
#define SPICON_RFLUSH_EN               (0x1   << 12 ) /* EN. If this bit is set, all incoming data is ignored and no interrupts are generated. If set and TIM = 0, a read of the Rx FIFO initiates a transfer. */
...
#define SPICON_RXOF_DIS                (0x0   << 8  ) /* DIS. The new serial byte received is discarded. */
#define SPICON_RXOF_EN                 (0x1   << 8  ) /* EN. The valid data in the Rx register is overwritten by the new serial byte received. */

/* SPICON[ZEN] - Transmit zeros when Tx FIFO is empty. */
#define SPICON_ZEN_MSK                 (0x1   << 7  )
#define SPICON_ZEN                     (0x1   << 7  )
#define SPICON_ZEN_DIS                 (0x0   << 7  ) /* DIS. Transmit the last transmitted value when there is no valid data in the Tx FIFO. */
#define SPICON_ZEN_EN                  (0x1   << 7  ) /* EN. Transmit 0x00 when there is no valid data in the Tx FIFO. */

/* SPICON[TIM] - Transfer and interrupt mode bit. */
#define SPICON_TIM_MSK                 (0x1   << 6  )
#define SPICON_TIM                     (0x1   << 6  )
#define SPICON_TIM_RXRD                (0x0   << 6  ) /* RXRD. Initiate transfer with a write to the SPIxTX register.  Interrupt only occurs when Tx is empty. */
#define SPICON_TIM_TXWR                (0x1   << 6  ) /* TXWR. Initiate transfer with a read of the SPIxRX register.  The read must be done while the SPI interface is idle. Interrupt only occurs when Rx is full. */
...
#define SPISTA_RXOF_SET                (0x1   << 7  ) /* SET. Set when the Rx FIFO is already full when new data is loaded to the FIFO. This bit generates an interrupt except when RFLUSH is set in SPI0CON. */
...
#define SPISTA_RX_SET                  (0x1   << 6  ) /* SET. Set when a receive interrupt occurs. This bit is set when TIM in SPI0CON is cleared and the required number of bytes have been received. */
...
#define SPISTA_TX_SET                  (0x1   << 5  ) /* SET. Set when a transmit interrupt occurs. This bit is set when TIM in SPI0CON is set and the required number of bytes have been transmitted. */
...
#define ADCCFG_REF_INTERNAL125V        (0x0   << 13 ) /* INTERNAL125V. Select the internal 1.25V reference as the ADC reference. */
...
#define ADCCON_REFBUF_EN               (0x0   << 7  ) /* EN. Turn on the reference buffer. The reference buffer takes 5 ms to settle and consumes approximately 210 µA. */
...
#define CLKCON_CLKMUX_EXTCLK           (0x3   << 3  ) /* EXTCLK. External clock on P0.5. */
...
#define CLKPD_T2_DIS                   (0x0   << 4  ) /* DIS. Dsable T2 clocks.   */
...
typedef struct {                            /*!< pADI_DMA Structure                     */
  ...
  __IO uint8_t   DMAPERID4;                 /*!< DMA Peripheral Identification 4 Register */
  __I  uint8_t   RESERVED4[15];
  __IO uint8_t   DMAPERID0;                 /*!< DMA Peripheral Identification 0 Register */
  __I  uint8_t   RESERVED5[3];
  __IO uint8_t   DMAPERID1;                 /*!< DMA Peripheral Identification 1 Register */
  __I  uint8_t   RESERVED6[3];
  __IO uint8_t   DMAPERID2;                 /*!< DMA Peripheral Identification 2 Register */
  __I  uint8_t   RESERVED7[3];
  __IO uint8_t   DMAPERID3;                 /*!< DMA Peripheral Identification 3 Register */
  __I  uint8_t   RESERVED8[3];
  __IO uint8_t   DMAPCELLID0;               /*!< DMA PrimeCell Identification 0 Register */
  __I  uint8_t   RESERVED9[3];
  __IO uint8_t   DMAPCELLID1;               /*!< DMA PrimeCell Identification 1 Register */
  __I  uint8_t   RESERVED10[3];
  __IO uint8_t   DMAPCELLID2;               /*!< DMA PrimeCell Identification 2 Register */
  __I  uint8_t   RESERVED11[3];
  __IO uint8_t   DMAPCELLID3;               /*!< DMA PrimeCell Identification 3 Register */
} ADI_DMA_TypeDef;
#else // (__NO_MMR_STRUCTS__==0)
...
#define DMASTA_ENABLE_DIS              (0x0   << 0  ) /* DIS. Controller is disabled. */
#define DMASTA_ENABLE_EN               (0x1   << 0  ) /* EN. Controller is enabled. */
...
/* DMAPERID4[JEP106CCODE] - The JEP106 continuation code value represents how many 0x7F continuation characters occur in the manufacturers identity code. */
#define DMAPERID4_JEP106CCODE_MSK      (0xF   << 0  )

/* Reset Value for DMAPERID0*/
#define DMAPERID0_RVAL                 0x30 

/* DMAPERID0[PARTNO0] - DMA Peripheral ID 0. */
...
/* DMAPERID1[PARTNO1] - DMA Peripheral ID 1. */
...
/* DMAPERID2[JEDECUSED] - This indicates that the controller uses a manufacturers identity code that was allocated by JEDEC according to JEP106. */
...
/* DMAPERID2[JEP106ID1] - DMA Peripheral ID 2. */
...
typedef struct {                            /*!< pADI_FEE Structure                     */
  ...
  __IO uint16_t  FEEPROL;                   /*!< Write Protection Register (Lower 16 bits) */
  __I  uint16_t  RESERVED8;
  __IO uint16_t  FEEPROH;                   /*!< Write Protection Register (Upper 16 bits) */
  ...
  __IO uint16_t  FEEADRAL;                  /*!< Abort Address Register (Lower 16 bits) */
  __I  uint16_t  RESERVED13;
  __IO uint16_t  FEEADRAH;                  /*!< Abort Address Register (Upper 16 bits) */
  ...
  __IO uint16_t  FEEAEN2;                   /*!< Interrupt Abort Register (Interrupt 47 to Interrupt 32) */
} ADI_FEE_TypeDef;
#else // (__NO_MMR_STRUCTS__==0)
...
/* FEESTA[SIGNERR] - Kernel space signature check on reset error. After reset, the flash controller automatically checks the information space signature. If the signature check fails, this bit is asserted. The user can check if this bit is set via serial wire only. User code does not execute if this bit is set. */
#define FEESTA_SIGNERR_BBA             (*(volatile unsigned long *) 0x42050018)
#define FEESTA_SIGNERR_MSK             (0x1   << 6  )
#define FEESTA_SIGNERR                 (0x1   << 6  )
#define FEESTA_SIGNERR_DIS             (0x0   << 6  ) /* DIS                      */
#define FEESTA_SIGNERR_EN              (0x1   << 6  ) /* EN                       */

/* FEESTA[CMDRES] - These two bits indicate the status of a command on completion or the status of a write via the AHB bus. If multiple commands are executed or there are multiple writes via the AHB bus without a read of the status register, then the first error encountered is stored. */
#define FEESTA_CMDRES_MSK              (0x3   << 4  )
#define FEESTA_CMDRES_SUCCESS          (0x0   << 4  ) /* SUCCESS. Indicates a successful completion of a command or a write. */
...
#define FEESTA_WRDONE_DIS              (0x0   << 3  ) /* DIS. Cleared to 0 when read. */
#define FEESTA_WRDONE_EN               (0x1   << 3  ) /* EN. Set to 1 when a write via the AHB bus completes. */
...
#define FEESTA_CMDDONE_DIS             (0x0   << 2  ) /* DIS. Cleared to 0 when read. */
#define FEESTA_CMDDONE_EN              (0x1   << 2  ) /* EN. Set to 1 when a command completes. */
...
#define FEESTA_WRBUSY_DIS              (0x0   << 1  ) /* DIS                      */
#define FEESTA_WRBUSY_EN               (0x1   << 1  ) /* EN. Set to 1 when the flash block is executing a write via the AHB bus. */
...
#define FEESTA_CMDBUSY_DIS             (0x0   << 0  ) /* DIS                      */
#define FEESTA_CMDBUSY_EN              (0x1   << 0  ) /* EN. Set to 1 when the flash block is executing any command entered via the command register. */
...
#define FEECON0_WREN_DIS               (0x0   << 2  ) /* DIS. Disable writing to the flash. */
#define FEECON0_WREN_EN                (0x1   << 2  ) /* EN. Enable writing to the flash. */
...
/* FEEADR0L[VALUE] - Bits[15:9] of the address of a page in flash. Used by the erase and sign commands. */
#define FEEADR0L_VALUE_MSK             (0xFFFF << 0  )

/* Reset Value for FEEADR0H*/
#define FEEADR0H_RVAL                  0x0 

/* FEEADR0H[VALUE] - Bits[15:9] of the address of a page in flash. Used by the erase and sign commands. */
#define FEEADR0H_VALUE_MSK             (0x3   << 0  )

/* Reset Value for FEEADR1L*/
#define FEEADR1L_RVAL                  0x0 

/* FEEADR1L[VALUE] - Bits[15:9] of the address of a page in flash. Used to identify the last page used by the Sign command. */
#define FEEADR1L_VALUE_MSK             (0xFFFF << 0  )

/* Reset Value for FEEADR1H*/
#define FEEADR1H_RVAL                  0x0 

/* FEEADR1H[VALUE] - Bit 16 of the page address. */
...
/* FEEPROL[VALUE] - Lower 16 bits of the write protection. */
#define FEEPROL_VALUE_MSK              (0xFFFF << 0  )

/* Reset Value for FEEPROH*/
#define FEEPROH_RVAL                   0xFFFF 

/* FEEPROH[VALUE] - Upper 16 bits of the write protection. */
...
#define FEEAEN2_PWM3_MSK               (0x1   << 9  )
#define FEEAEN2_PWM3                   (0x1   << 9  )
#define FEEAEN2_PWM3_DIS               (0x0   << 9  ) /* DIS                      */
#define FEEAEN2_PWM3_EN                (0x1   << 9  ) /* EN                       */

/* FEEAEN2[PWM2] -  */
#define FEEAEN2_PWM2_BBA               (*(volatile unsigned long *) 0x42051020)
#define FEEAEN2_PWM2_MSK               (0x1   << 8  )
#define FEEAEN2_PWM2                   (0x1   << 8  )
#define FEEAEN2_PWM2_DIS               (0x0   << 8  ) /* DIS                      */
#define FEEAEN2_PWM2_EN                (0x1   << 8  ) /* EN                       */

/* FEEAEN2[PWM1] -  */
#define FEEAEN2_PWM1_BBA               (*(volatile unsigned long *) 0x4205101C)
#define FEEAEN2_PWM1_MSK               (0x1   << 7  )
#define FEEAEN2_PWM1                   (0x1   << 7  )
#define FEEAEN2_PWM1_DIS               (0x0   << 7  ) /* DIS                      */
#define FEEAEN2_PWM1_EN                (0x1   << 7  ) /* EN                       */

/* FEEAEN2[PWM0] -  */
#define FEEAEN2_PWM0_BBA               (*(volatile unsigned long *) 0x42051018)
#define FEEAEN2_PWM0_MSK               (0x1   << 6  )
#define FEEAEN2_PWM0                   (0x1   << 6  )
#define FEEAEN2_PWM0_DIS               (0x0   << 6  ) /* DIS                      */
#define FEEAEN2_PWM0_EN                (0x1   << 6  ) /* EN                       */

/* FEEAEN2[PWMTRIP] -  */
#define FEEAEN2_PWMTRIP_BBA            (*(volatile unsigned long *) 0x42051014)
#define FEEAEN2_PWMTRIP_MSK            (0x1   << 5  )
#define FEEAEN2_PWMTRIP                (0x1   << 5  )
#define FEEAEN2_PWMTRIP_DIS            (0x0   << 5  ) /* DIS                      */
#define FEEAEN2_PWMTRIP_EN             (0x1   << 5  ) /* EN                       */

/* FEEAEN2[DMASPI0RX] -  */
#define FEEAEN2_DMASPI0RX_BBA          (*(volatile unsigned long *) 0x42051010)
#define FEEAEN2_DMASPI0RX_MSK          (0x1   << 4  )
#define FEEAEN2_DMASPI0RX              (0x1   << 4  )
#define FEEAEN2_DMASPI0RX_DIS          (0x0   << 4  ) /* DIS                      */
#define FEEAEN2_DMASPI0RX_EN           (0x1   << 4  ) /* EN                       */

/* FEEAEN2[DMASPI0TX] -  */
#define FEEAEN2_DMASPI0TX_BBA          (*(volatile unsigned long *) 0x4205100C)
#define FEEAEN2_DMASPI0TX_MSK          (0x1   << 3  )
#define FEEAEN2_DMASPI0TX              (0x1   << 3  )
#define FEEAEN2_DMASPI0TX_DIS          (0x0   << 3  ) /* DIS                      */
#define FEEAEN2_DMASPI0TX_EN           (0x1   << 3  ) /* EN                       */

/* FEEAEN2[DMAADC] -  */
#define FEEAEN2_DMAADC_BBA             (*(volatile unsigned long *) 0x42051008)
#define FEEAEN2_DMAADC_MSK             (0x1   << 2  )
#define FEEAEN2_DMAADC                 (0x1   << 2  )
#define FEEAEN2_DMAADC_DIS             (0x0   << 2  ) /* DIS                      */
#define FEEAEN2_DMAADC_EN              (0x1   << 2  ) /* EN                       */
// ------------------------------------------------------------------------------------------------
// -----                                        GPIO0                                        -----
// ------------------------------------------------------------------------------------------------
...
typedef struct {                            /*!< pADI_GP0 Structure                     */
  ...
  __I  uint8_t   RESERVED1[3];
  __IO uint8_t   GPPUL;                     /*!< GPIO Port 0 OutputPull Up Enable      */
  __I  uint8_t   RESERVED2[3];
...
/* GP0CON[CON7] - Configuration bits for Px.7 (not available for port GPIO1). */
...
/* GP0CON[CON6] - Configuration bits for Px.6. */
...
/* GP0PUL[PUL7] - Pullup enable for port pin. */
...
/* GP0PUL[PUL6] - Pullup enable for port pin. */
...
/* GP0PUL[PUL5] - Pullup enable for port pin. */
...
/* GP0PUL[PUL4] - Pullup enable for port pin. */
...
/* GP0PUL[PUL3] - Pullup enable for port pin. */
...
/* GP0PUL[PUL2] - Pullup enable for port pin. */
...
/* GP0PUL[PUL1] - Pullup enable for port pin. */
...
/* GP0PUL[PUL0] - Pullup enable for port pin. */
...
typedef struct {                            /*!< pADI_I2C Structure                     */
  ...
  __I  uint16_t  RESERVED1;
  __IO uint8_t   I2CMRX;                    /*!< Master Receive Data Register          */
  __I  uint8_t   RESERVED2[3];
  __IO uint8_t   I2CMTX;                    /*!< Master Transmit Data Register         */
...
#define I2CMCON_TXDMA_DIS              (0x0   << 11 ) /* DIS. Disable DMA mode.   */
...
#define I2CMCON_RXDMA_DIS              (0x0   << 10 ) /* DIS. Disable DMA mode.   */
...
#define I2CMCON_IENCMP_DIS             (0x0   << 8  ) /* DIS. Disable.            */
#define I2CMCON_IENCMP_EN              (0x1   << 8  ) /* EN. When asserted an interrupt is generated when a STOP is detected. */
...
#define I2CMCON_IENNACK_DIS            (0x0   << 7  ) /* DIS. Disable             */
#define I2CMCON_IENNACK_EN             (0x1   << 7  ) /* EN. Enable.              */
...
#define I2CMCON_IENALOST_DIS           (0x0   << 6  ) /* DIS. Disable             */
#define I2CMCON_IENALOST_EN            (0x1   << 6  ) /* EN. Enable.              */
...
#define I2CMCON_IENTX_DIS              (0x0   << 5  ) /* DIS. Disable.            */
#define I2CMCON_IENTX_EN               (0x1   << 5  ) /* EN. Enable.              */
...
#define I2CMCON_IENRX_DIS              (0x0   << 4  ) /* DIS. Disable.            */
#define I2CMCON_IENRX_EN               (0x1   << 4  ) /* EN. Enable.              */
...
#define I2CMCON_STRETCH_EN             (0x1   << 3  ) /* EN. Setting this bit instructs the device that if SCL is 0, hold it at 0. Or if SCL is 1, then when it next goes to 0, hold it at 0. */
...
#define I2CMCON_LOOPBACK_EN            (0x1   << 2  ) /* EN. SCL and SDA out of the device are muxed onto their corresponding inputs. Note that is also possible for the master to loop back a transfer to the slave as long as the device address corresponds, that is, external loopback. */
...
/* I2CMCON[MASEN] - Master enable bit. */
#define I2CMCON_MASEN_BBA              (*(volatile unsigned long *) 0x42060000)
#define I2CMCON_MASEN_MSK              (0x1   << 0  )
#define I2CMCON_MASEN                  (0x1   << 0  )
#define I2CMCON_MASEN_DIS              (0x0   << 0  ) /* DIS. Master state machine flops are held in reset and the master is disabled. The master should be disabled when not in use because this gates the clock to the master and save power. This bit should not be cleared until a transaction has completed (see the TCCOMP bit in the master status register). */
#define I2CMCON_MASEN_EN               (0x1   << 0  ) /* EN. Enable master.       */
...
#define I2CMSTA_TXUR_DIS               (0x0   << 12 ) /* DIS. Disable.            */
#define I2CMSTA_TXUR_EN                (0x1   << 12 ) /* EN. Set to 1 when the I2C master ends the transaction due to a Tx FIFO empty condition.  This bit is only set when IENTX (I2CSCON[10]) is set. */
...
#define I2CMSTA_MSTOP_DIS              (0x0   << 11 ) /* DIS. Disable.            */
#define I2CMSTA_MSTOP_EN               (0x1   << 11 ) /* EN. Set to 1 when the I2C master drives a stop condition on the I2C bus, therefore indicating a transaction completion, Tx underrun, Rx overflow, or a NACK by the slave. It is different from TCOMP because it is not set when the stop condition occurs due to any other master on the I2C bus.  This bit does not generate an interrupt. See the TCOMP description for available interrupts related to the stop condition. */
...
#define I2CMSTA_LINEBUSY_DIS           (0x0   << 10 ) /* DIS. Cleared to 0 when a stop is detected on the I2C bus. */
#define I2CMSTA_LINEBUSY_EN            (0x1   << 10 ) /* EN. Set to 1 when a start is detected on the I2C bus. */
...
#define I2CMSTA_RXOF_DIS               (0x0   << 9  ) /* DIS. Disable.            */
#define I2CMSTA_RXOF_EN                (0x1   << 9  ) /* EN. Set to 1 when a byte is written to the receive FIFO when the FIFO is already full. */
...
#define I2CMSTA_TCOMP_DIS              (0x0   << 8  ) /* DIS. Disable.            */
#define I2CMSTA_TCOMP_EN               (0x1   << 8  ) /* EN. Set to 1 when a STOP condition is detected on the I2C bus.  If IENCMP is 1, an interrupt is generated when this bit asserts. This bit only asserts if the master is enabled (MASEN = 1).  This bit should be used to determine when it is safe to disable the master. It can also be used to wait for another master's transaction to complete on the I2C bus when this master loses arbitration. */
...
#define I2CMSTA_NACKDATA_DIS           (0x0   << 7  ) /* DIS. Cleared on a read of the I2CMSTA register. */
#define I2CMSTA_NACKDATA_EN            (0x1   << 7  ) /* EN. Set to 1 when a NACK is received in response to a data write transfer. If IENNACK is 1, an interrupt is generated when this bit asserts. */
...
#define I2CMSTA_BUSY_DIS               (0x0   << 6  ) /* DIS. Cleared to 0 if the state machine is idle or another device has control of the I2C bus. */
#define I2CMSTA_BUSY_EN                (0x1   << 6  ) /* EN. Set to 1 when the master state machine is servicing a transaction. */
...
#define I2CMSTA_ALOST_DIS              (0x0   << 5  ) /* DIS. Cleared to 0 on a read of the I2CMSTA register. */
#define I2CMSTA_ALOST_EN               (0x1   << 5  ) /* EN. Set to 1 If the master looses arbitration.  If IENALOST is 1, an interrupt is generated when this bit asserts. */
...
#define I2CMSTA_NACKADDR_DIS           (0x0   << 4  ) /* DIS. Cleared to 0 on a read of the I2CMSTA register. */
#define I2CMSTA_NACKADDR_EN            (0x1   << 4  ) /* EN. Set to 1 if a NACK received in response to an address.  If IENNACK is 1, an interrupt is generated when this bit asserts. */
...
#define I2CMSTA_RXREQ_DIS              (0x0   << 3  ) /* DIS. Disable.            */
#define I2CMSTA_RXREQ_EN               (0x1   << 3  ) /* EN. Set to 1 when there is data in the receive FIFO.  If IENRX is 1, an interrupt is generated when this bit asserts. */
...
#define I2CMSTA_TXREQ_DIS              (0x0   << 2  ) /* DIS. Disable.            */
#define I2CMSTA_TXREQ_EN               (0x1   << 2  ) /* EN. Set to 1 when the direction bit is 0 and the transmit FIFO is either empty or not full. If IENTX is 1, an interrupt is generated when this bit asserts. */
...
/* I2CMRX[VALUE] - Receive register. Read only. 0 by default. This register allows access to the receive data FIFO.  The FIFO can hold two bytes. */
#define I2CMRX_VALUE_MSK               (0xFF  << 0  )

/* Reset Value for I2CMTX*/
#define I2CMTX_RVAL                    0x0 

/* I2CMTX[VALUE] - Transmit register. 0 by default. This register allows access to the transmit data FIFO. The FIFO can hold two bytes. */
#define I2CMTX_VALUE_MSK               (0xFF  << 0  )

/* Reset Value for I2CMRXCNT*/
#define I2CMRXCNT_RVAL                 0x0 

/* I2CMRXCNT[EXTEND] - Extended read: Use this bit if greater than 256 bytes are required on a read.  For example: To receive 412 bytes, write 0x100 (EXTEND = 1) to this register (I2CMRXCNT).  Wait for the first byte to be received, then check the I2CMCRXCNT register for every byte received thereafter.  When I2CMCRXCNT returns to 0, 256 bytes have been received.  Then, write 0x09C (412 - 256 = 156 decimal (equal to 0x9C) – with the EXTEND bt set to 0) to this register (I2CMRXCNT). */
...
/* I2CDIV[HIGH] - Serial clock high time. This register controls the clock high time. The timer is driven by UCLK.  Use the following equation to derive the required high time: HIGH = (REQD_HIGH_TIME/UCLK_PERIOD) - 2 See the following example: For example to generate a 400kHz SCL with a low time of 1250 ns and a high time of 1250 ns, with a UCLK frequency of 16 MHz: HIGH = 1250 ns/62.5n s - 2 = 0x12 This register is reset to 0x1F, which gives an SCL high time of 33 UCLK ticks. */
#define I2CDIV_HIGH_MSK                (0xFF  << 8  )

/* I2CDIV[LOW] - Serial clock low time. This register controls the clock low time. The timer is driven by the core clock (UCLK). Use the following equation to derive the required low time. LOW = (REQD_LOW_TIME/UCLK_PERIOD) - 1 For example to generate a 400kHz SCL with a low time of 1250ns and a high time of 1250ns, with a UCLK frequency of 16 MHz: LOW = 1250 ns/62.5 ns - 1 = 0x13 This register is reset to 0x1F, which gives an SCL low time of 32 UCLK ticks. */
...
/* I2CSCON[XMITDEC] - Decrement status when byte has been transmitted */
#define I2CSCON_XMITDEC_BBA            (*(volatile unsigned long *) 0x4206052C)
#define I2CSCON_XMITDEC_MSK            (0x1   << 11 )
#define I2CSCON_XMITDEC                (0x1   << 11 )
#define I2CSCON_XMITDEC_DIS            (0x0   << 11 ) /* DIS                      */
#define I2CSCON_XMITDEC_EN             (0x1   << 11 ) /* EN                       */
...
#define I2CSCON_IENSTOP_EN             (0x1   << 8  ) /* EN.  Enable stop condition detect interrupt. */
...
/* I2CSCON[STRETCH] - Stretch SCL enable. */
...
#define I2CSCON_STRETCH_EN             (0x1   << 6  ) /* EN. Tell the device that, if SCL is 0, hold it at 0.  Or if SCL is 1, then when it next goes to 0 hold it at 0. */
...
#define I2CSCON_EARLYTXR_EN            (0x1   << 5  ) /* EN. Enable a transmit request just after the positive edge of the direction bit (R/W) SCL clock pulse. */
...
#define I2CSCON_GCSBCLR_CLR            (0x1   << 4  ) /* CLR. Clear the General Call status and General Call ID bits  The General Call status and General Call ID bits are not reset by anything other than a write to this bit or a full reset. */
...
#define I2CSCON_GCEN_DIS               (0x0   << 2  ) /* DIS. Disable.            */
#define I2CSCON_GCEN_EN                (0x1   << 2  ) /* EN. Enable the I2C slave to ACK an I2C general call, Address 0x00 (write). */

/* I2CSCON[ADR10EN] - Enable 10 bit addressing. */
#define I2CSCON_ADR10EN_BBA            (*(volatile unsigned long *) 0x42060504)
#define I2CSCON_ADR10EN_MSK            (0x1   << 1  )
#define I2CSCON_ADR10EN                (0x1   << 1  )
#define I2CSCON_ADR10EN_DIS            (0x0   << 1  ) /* DIS. If this bit is clear, the slave can support four slave addresses, programmed in Registers I2CID0 to I2CID3. */
#define I2CSCON_ADR10EN_EN             (0x1   << 1  ) /* EN. Enable 10-bit addressing. One 10-bit address is supported by the slave and is stored in I2CID0 and I2CID1, where I2CID0 contains the first byte of the address and the upper five bits must be programmed to 11110' I2CID2 and I2CID3 can be programmed with 7-bit addresses at the same time. */
...
#define I2CSCON_SLVEN_DIS              (0x0   << 0  ) /* DIS. Disable the slave and all slave state machine flops are held in reset. */
#define I2CSCON_SLVEN_EN               (0x1   << 0  ) /* EN. Enable slave.        */
...
#define I2CSSTA_START_DIS              (0x0   << 14 ) /* DIS. Cleared to 0 on receipt of either a stop or start condition. */
#define I2CSSTA_START_EN               (0x1   << 14 ) /* EN. Set to 1 if a start is detected on SCL/SDA and one of the following: The device address is matched.  A general call (GC = 0000_0000) code is received and GC is enabled.  A high speed (HS = 0000_1XXX) code is received. A start byte (0000_0001) is received. */
...
#define I2CSSTA_REPSTART_DIS           (0x0   << 13 ) /* DIS. Cleared to 0 when read or on receipt of a stop condition. */
#define I2CSSTA_REPSTART_EN            (0x1   << 13 ) /* EN. Set to 1 if START (I2CSSTA[14]) is already asserted and then a repeated start is detected. */
...
#define I2CSSTA_STOP_DIS               (0x0   << 10 ) /* DIS. Cleared to 0 by a read of the status register. */
#define I2CSSTA_STOP_EN                (0x1   << 10 ) /* EN. Set to 1 if the slave device received a stop condition after a previous start condition and a matching address. */
...
#define I2CSSTA_GCINT_DIS              (0x0   << 7  ) /* DIS. To clear write 1 to the I2CSCON[4].  If it was a general call reset, all registers are at their default values. If it was a hardware general call, the Rx FIFO holds the second byte of the general call and this can be compared with the ALT register. */
#define I2CSSTA_GCINT_EN               (0x1   << 7  ) /* EN. Set to 1 if the slave device receives a general call of any type. */
...
#define I2CSSTA_BUSY_DIS               (0x0   << 6  ) /* DIS. Cleared to 0 by hardware on the following conditions: The address does not match an ID register. The slave device receives a I2C stop condition. If a repeated start address doesn’t match. */
#define I2CSSTA_BUSY_EN                (0x1   << 6  ) /* EN. Set to 1 if the slave device receives an I2C start condition. */
...
#define I2CSSTA_NOACK_DIS              (0x0   << 5  ) /* DIS. Cleared to 0 on a read of the I2CSSTA register. */
#define I2CSSTA_NOACK_EN               (0x1   << 5  ) /* EN. Set to 1 to indicate that the slave responded to its device address with a NACK. Set under the following conditions: If there was no data to transmit and sequence was a slave read, the device address is NACK'ed. If the NACK bit was set in the slave control register and the device was addressed. */
...
#define I2CSSTA_RXOF_DIS               (0x0   << 4  ) /* DIS.                     */
#define I2CSSTA_RXOF_EN                (0x1   << 4  ) /* EN. Set to 1 when a byte is written to the receive FIFO when the FIFO is already full. */
...
#define I2CSSTA_RXREQ_DIS              (0x0   << 3  ) /* DIS. Cleared to 0 when the receive FIFO is read or flushed. */
#define I2CSSTA_RXREQ_EN               (0x1   << 3  ) /* EN. Set to 1 when the receive FIFO is not empty. Set on the falling edge of the SCL clock pulse that clocks in the last data bit of a byte. */
...
#define I2CSSTA_TXUR_DIS               (0x0   << 1  ) /* DIS. Disable.            */
#define I2CSSTA_TXUR_EN                (0x1   << 1  ) /* EN. Set to 1 if a master requests data from the device and the Tx FIFO is empty for the rising edge of SCL. */
...
#define I2CSSTA_TXFSEREQ_DIS           (0x0   << 0  ) /* DIS. Disable.            */
#define I2CSSTA_TXFSEREQ_EN            (0x1   << 0  ) /* EN. Set to 1 whenever the slave Tx FIFO is empty. */
...
/* PWRMOD[WICENACK] - WIC Acknowledge */
#define PWRMOD_WICENACK_BBA            (*(volatile unsigned long *) 0x4204800C)
#define PWRMOD_WICENACK_MSK            (0x1   << 3  )
#define PWRMOD_WICENACK                (0x1   << 3  )
#define PWRMOD_WICENACK_DIS            (0x0   << 3  ) /* DIS. Cleared by the cortex after writing any PWRMOD bit except PWRMOD[5] or PWRMOD[6]. */
#define PWRMOD_WICENACK_EN             (0x1   << 3  ) /* EN. Set by the cortex after writing PWRMOD[5] or PWRMOD[6] and Cortex not ready. */
...
/* PSMCON[VBATEN] - VBAT monitor enable bit */
#define PSMCON_VBATEN_BBA              (*(volatile unsigned long *) 0x42048104)
#define PSMCON_VBATEN_MSK              (0x1   << 1  )
#define PSMCON_VBATEN                  (0x1   << 1  )
#define PSMCON_VBATEN_EN               (0x0   << 1  ) /* EN enables the power supply (VBAT) monitor. */
#define PSMCON_VBATEN_DIS              (0x1   << 1  ) /* DIS disables the power supply (VBAT) monitor. */

/* PSMCON[LVDD1EN] - 1.8V LDO monitor enable bit */
#define PSMCON_LVDD1EN_BBA             (*(volatile unsigned long *) 0x42048100)
#define PSMCON_LVDD1EN_MSK             (0x1   << 0  )
#define PSMCON_LVDD1EN                 (0x1   << 0  )
#define PSMCON_LVDD1EN_EN              (0x0   << 0  ) /* EN enables the 1.8V LDO (LVDD1) monitor. */
#define PSMCON_LVDD1EN_DIS             (0x1   << 0  ) /* DIS disables the 1.8V LDO (LVDD1) monitor. */
...
#define SRAMRET_RETAIN_DIS             (0x0   << 0  ) /* DIS to retain contents of the bottom 8kB of SRAM only */
#define SRAMRET_RETAIN_EN              (0x1   << 0  ) /* EN to retain contents of the entire 16kB of SRAM */
...
#define SHUTDOWN_EINT8_CLR             (0x0   << 2  ) /* CLR indicates the interrupt was not detected */
...
#define SHUTDOWN_EINT1_CLR             (0x0   << 1  ) /* CLR indicates the interrupt was not detected */
...
#define SHUTDOWN_EINT0_CLR             (0x0   << 0  ) /* CLR indicates the interrupt was not detected */
...
#define PWMCON0_SYNC_DIS               (0x0   << 15 ) /* DIS. Ignore transitions on the SYNC pin. */
#define PWMCON0_SYNC_EN                (0x1   << 15 ) /* EN. All PWM counters are reset on the next clock edge after the detection of a falling edge on the SYNC pin. */

/* PWMCON0[PWM7INV] - Inversion of PWM output. */
...
#define PWMCON0_HOFF_EN                (0x1   << 4  ) /* EN. Force PWM0 and PWM2 outputs high and PWM1 and PWM3 low (default). */
...
#define PWMCON0_HMODE_DIS              (0x0   << 1  ) /* DIS. PWMs in standard mode. */
#define PWMCON0_HMODE_EN               (0x1   << 1  ) /* EN. Enables H-Bridge mode and Bits[5:2] of PWMCON0 (default). */
...
/* PWMCON1[CONVSTARTEN] - Enable ADC conversion start from PWM. */
#define PWMCON1_CONVSTARTEN_BBA        (*(volatile unsigned long *) 0x4202009C)
#define PWMCON1_CONVSTARTEN_MSK        (0x1   << 7  )
#define PWMCON1_CONVSTARTEN            (0x1   << 7  )
#define PWMCON1_CONVSTARTEN_DIS        (0x0   << 7  ) /* DIS. Disable ADC Conversion from PWM. */
#define PWMCON1_CONVSTARTEN_EN         (0x1   << 7  ) /* EN.  Enable ADC Conversion from PWM. */
...
#define PWMCLRI_TRIP_EN                (0x1   << 4  ) /* EN. Clear the latched IRQPWM0 interrupt. */

/* PWMCLRI[PWM3] - Clear the latched PWM3 interrupt. This bit always reads 0. */
#define PWMCLRI_PWM3_BBA               (*(volatile unsigned long *) 0x4202010C)
#define PWMCLRI_PWM3_MSK               (0x1   << 3  )
#define PWMCLRI_PWM3                   (0x1   << 3  )
#define PWMCLRI_PWM3_EN                (0x1   << 3  ) /* EN. Clear the latched IRQPWM1 interrupt. */

/* PWMCLRI[PWM2] - Clear the latched PWM2 interrupt. This bit always reads 0. */
#define PWMCLRI_PWM2_BBA               (*(volatile unsigned long *) 0x42020108)
#define PWMCLRI_PWM2_MSK               (0x1   << 2  )
#define PWMCLRI_PWM2                   (0x1   << 2  )
#define PWMCLRI_PWM2_EN                (0x1   << 2  ) /* EN. Clear the latched IRQPWM2 interrupt. */

/* PWMCLRI[PWM1] - Clear the latched PWM1 interrupt. This bit always reads 0. */
#define PWMCLRI_PWM1_BBA               (*(volatile unsigned long *) 0x42020104)
#define PWMCLRI_PWM1_MSK               (0x1   << 1  )
#define PWMCLRI_PWM1                   (0x1   << 1  )
#define PWMCLRI_PWM1_EN                (0x1   << 1  ) /* EN. Clear the latched IRQPWM3 interrupt. */

/* PWMCLRI[PWM0] - Clear the latched PWM0 interrupt. This bit always reads 0. */
#define PWMCLRI_PWM0_BBA               (*(volatile unsigned long *) 0x42020100)
#define PWMCLRI_PWM0_MSK               (0x1   << 0  )
#define PWMCLRI_PWM0                   (0x1   << 0  )
#define PWMCLRI_PWM0_EN                (0x1   << 0  ) /* EN. Clear the latched TRIPIRQPWM interrupt. */
...
#define SPI0STA_RXOF_SET               (0x1   << 7  ) /* SET. Set when the Rx FIFO is already full when new data is loaded to the FIFO. This bit generates an interrupt except when RFLUSH is set in SPI0CON. */
...
#define SPI0STA_RX_SET                 (0x1   << 6  ) /* SET. Set when a receive interrupt occurs. This bit is set when TIM in SPI0CON is cleared and the required number of bytes have been received. */
...
#define SPI0STA_TX_SET                 (0x1   << 5  ) /* SET. Set when a transmit interrupt occurs. This bit is set when TIM in SPI0CON is set and the required number of bytes have been transmitted. */
...
#define SPI0CON_RFLUSH_EN              (0x1   << 12 ) /* EN. If this bit is set, all incoming data is ignored and no interrupts are generated. If set and TIM = 0, a read of the Rx FIFO initiates a transfer. */
...
#define SPI0CON_RXOF_DIS               (0x0   << 8  ) /* DIS. The new serial byte received is discarded. */
#define SPI0CON_RXOF_EN                (0x1   << 8  ) /* EN. The valid data in the Rx register is overwritten by the new serial byte received. */

/* SPI0CON[ZEN] - Transmit zeros when Tx FIFO is empty. */
#define SPI0CON_ZEN_BBA                (*(volatile unsigned long *) 0x4208021C)
#define SPI0CON_ZEN_MSK                (0x1   << 7  )
#define SPI0CON_ZEN                    (0x1   << 7  )
#define SPI0CON_ZEN_DIS                (0x0   << 7  ) /* DIS. Transmit the last transmitted value when there is no valid data in the Tx FIFO. */
#define SPI0CON_ZEN_EN                 (0x1   << 7  ) /* EN. Transmit 0x00 when there is no valid data in the Tx FIFO. */
...
#define SPI0CON_TIM_RXRD               (0x0   << 6  ) /* RXRD. Initiate transfer with a write to the SPIxTX register.  Interrupt only occurs when Tx is empty. */
#define SPI0CON_TIM_TXWR               (0x1   << 6  ) /* TXWR. Initiate transfer with a read of the SPIxRX register.  The read must be done while the SPI interface is idle. Interrupt only occurs when Rx is full. */
...
#define SPI1STA_CSERR_DIS              (0x0   << 12 ) /* CLR: Cleared when no CS error is detected. Cleared to 0 on a read of SPI1STA register. */
#define SPI1STA_CSERR_EN               (0x1   << 12 ) /* EN. Set when the CS line is deasserted abruptly. */
...
#define SPI1STA_RXS_DIS                (0x0   << 11 ) /* CLR. When the number of bytes in the FIFO is equal or less than the number in SPI0CON[15:14]. This bit is not cleared on a read of SPI1STA register. */
#define SPI1STA_RXS_EN                 (0x1   << 11 ) /* SET. When there are more bytes in the Rx FIFO than configured in MOD (SPI1CON[15:14]). For example if MOD = TX1RX1, RXS is set when there are 2 or more bytes in the Rx FIFO. This bit does not dependent on SPI1CON[6] and does not cause an interrupt. */
...
#define SPI1STA_RXOF_DIS               (0x0   << 7  ) /* DIS.Cleared to 0 on a read of SPI1STA register. */
#define SPI1STA_RXOF_EN                (0x1   << 7  ) /* EN. Set when the Rx FIFO is already full when new data is loaded to the FIFO. This bit generates an interrupt except when RFLUSH is set in SPI1CON. */
...
#define SPI1STA_RX_DIS                 (0x0   << 6  ) /* DIS. Cleared to 0 on a read of SPI1STA register. */
#define SPI1STA_RX_EN                  (0x1   << 6  ) /* EN. Set when a receive interrupt occurs. This bit is set when TIM in SPI0CON is cleared and the required number of bytes have been received. */
...
#define SPI1STA_TX_DIS                 (0x0   << 5  ) /* DIS. Cleared to 0 on a read of SPI1STA register. */
#define SPI1STA_TX_EN                  (0x1   << 5  ) /* SET. Set when a transmit interrupt occurs. This bit is set when TIM in SPI1CON is set and the required number of bytes have been transmitted. */
...
#define SPI1STA_TXUR_DIS               (0x0   << 4  ) /* CLR. Cleared to 0 on a read of SPI1STA register. */
#define SPI1STA_TXUR_EN                (0x1   << 4  ) /* SET. Set when a transmit is initiated without any valid data in the Tx FIFO. This bit generates an interrupt except when TFLUSH is set in SPI1CON. */
...
#define SPI1DIV_BCRST_EN               (0x1   << 7  ) /* EN. Enabled for a clean restart of SPI transfer after a CSERR condition. User code must also clear the SPI enable bit in SPI0CON during the CSERR interrupt. */
...
#define SPI1CON_MOD_TX2RX1             (0x1   << 14 ) /* TX2RX1.  Tx/Rx interrupt occurs when 2 bytes have been transmitted/received from/into the FIFO. */
...
#define SPI1CON_RFLUSH_EN              (0x1   << 12 ) /* EN.  If this bit is set, all incoming data is ignored and no interrupts are generated. If set and TIM = 0, a read of the Rx FIFO initiates a transfer. */
...
#define SPI1CON_RXOF_DIS               (0x0   << 8  ) /* DIS. The new serial byte received is discarded. */
#define SPI1CON_RXOF_EN                (0x1   << 8  ) /* EN. The valid data in the Rx register is overwritten by the new serial byte received. */

/* SPI1CON[ZEN] - Transmit zeros when Tx FIFO is empty. */
#define SPI1CON_ZEN_BBA                (*(volatile unsigned long *) 0x4208821C)
#define SPI1CON_ZEN_MSK                (0x1   << 7  )
#define SPI1CON_ZEN                    (0x1   << 7  )
#define SPI1CON_ZEN_DIS                (0x0   << 7  ) /* DIS. Transmit the last transmitted value when there is no valid data in the Tx FIFO. */
#define SPI1CON_ZEN_EN                 (0x1   << 7  ) /* EN. Transmit 0x00 when there is no valid data in the Tx FIFO. */
...
#define T0CON_EVENT_HFT                (0xB   << 8  ) /* Reserved.                */
...
#define T0CON_PRE_DIV1                 (0x0   << 0  ) /* DIV1. Source clock/1.    */
...
#define T1CON_EVENT_RESERVED           (0x5   << 8  ) /* Reserved.                */
...
#define T1CON_PRE_DIV1                 (0x0   << 0  ) /* DIV1. Source clock/1.    */
...
#define COMIEN_ETBEI_EN                (0x1   << 1  ) /* EN. Enable.              */
...
#define COMIEN_ERBFI_EN                (0x1   << 0  ) /* EN. Enable.              */
...
#define COMIIR_NINT_DIS                (0x0   << 0  ) /* DIS. Indicates any of the following: receive buffer full, transmit buffer empty, line status, or modem status interrupt occurred. */
#define COMIIR_NINT_EN                 (0x1   << 0  ) /* EN.There is no interrupt (default). */
...
#define COMLCR_BRK_CLR                 (0x0   << 6  ) /* CLR. Cleared to operate in normal mode. */
#define COMLCR_BRK_SET                 (0x1   << 6  ) /* SET. Set by user to force TxD to 0. */
...
#define COMMCR_OUT1_DIS                (0x0   << 3  ) /* DIS. No parity transmission or checking. */
#define COMMCR_OUT1_EN                 (0x1   << 3  ) /* EN.  Transmit and check the parity bit. */
...
#define COMMCR_OUT2_DIS                (0x0   << 2  ) /* DIS. Generate one stop bit in the transmitted data. */
#define COMMCR_OUT2_EN                 (0x1   << 2  ) /* EN. Transmit 1.5 stop bits if the word length is 5 bits, or 2 stop bits if the word length is 6, 7, or 8 bits. The receiver checks the first stop bit only, regardless of the number of stop bits selected. */
...
#define COMMCR_RTS_DIS                 (0x0   << 1  ) /* DIS. Force the RTS output to 1 should say logic high?). */
...
#define COMLSR_TEMT_DIS                (0x0   << 6  ) /* DIS. Cleared when writing to COMTX. */
#define COMLSR_TEMT_EN                 (0x1   << 6  ) /* EN. If COMTX and the shift register are empty, this bit indicates that the data has been transmitted, that is, it is no longer present in the shift register (default). */
...
#define COMLSR_THRE_DIS                (0x0   << 5  ) /* DIS. Cleared when writing to COMTX. */
#define COMLSR_THRE_EN                 (0x1   << 5  ) /* EN. If COMTX is empty, COMTX can be written as soon as this bit is set. The previous data may not have been transmitted yet and can still be present in the shift register (default). */
...
#define COMLSR_BI_DIS                  (0x0   << 4  ) /* DIS. Cleared automatically. */
#define COMLSR_BI_EN                   (0x1   << 4  ) /* EN. Set when UART RXD is held low for more than the maximum word length. */
...
#define COMLSR_FE_DIS                  (0x0   << 3  ) /* DIS. Cleared automatically. */
#define COMLSR_FE_EN                   (0x1   << 3  ) /* EN. Set when the stop bit is invalid. */
...
#define COMLSR_PE_DIS                  (0x0   << 2  ) /* DIS. Cleared automatically. */
#define COMLSR_PE_EN                   (0x1   << 2  ) /* EN. Set when a parity error occurs. */
...
#define COMLSR_OE_DIS                  (0x0   << 1  ) /* DIS. Cleared automatically. */
#define COMLSR_OE_EN                   (0x1   << 1  ) /* EN. Set automatically if data is overwritten before being read. */
...
#define COMLSR_DR_DIS                  (0x0   << 0  ) /* DIS. Cleared by reading COMRX. */
#define COMLSR_DR_EN                   (0x1   << 0  ) /* EN. Set automatically when COMRX is full. */
...
#define COMMSR_CTS_DIS                 (0x0   << 4  ) /* DIS.Cleared when CTS is currently logic low. */
#define COMMSR_CTS_EN                  (0x1   << 4  ) /* EN. Set when CTS is currently logic high. */
...
typedef struct {                            /*!< pADI_WUT Structure                     */
  __IO uint16_t  T2VAL0;                    /*!< Current Wake-Up Timer Value Bits 15 to 0 */
  __I  uint16_t  RESERVED0;
  __IO uint16_t  T2VAL1;                    /*!< Current Wake-Up Timer Value Bits 31 to 16 */
  __I  uint16_t  RESERVED1;
...
#define T2CON_PRE_DIV1                 (0x0   << 0  ) /* DIV1. Source clock/1.    */
...
#define T2STA_CON_SET                  (0x1   << 8  ) /* SET. This bit is set high when the Enable bit (bit 5) in the Control register is set or cleared. */
...
#define T3STA_IRQ_SET                  (0x1   << 0  ) /* SET. A timeout event has ocurred. */
/********************************************
...
// ------------------------------------------------------------------------------------------------
#define ADI_ADC0_ADDR                            0x40050000
...
#define ADI_GP4_ADDR                             0x400060C0
#define ADI_MISC_ADDR                            0x40008820
#define ADI_I2C_ADDR                             0x40003000
#define ADI_INTERRUPT_ADDR                       0x40002420
#define ADI_IDENT_ADDR                           0x40002020
