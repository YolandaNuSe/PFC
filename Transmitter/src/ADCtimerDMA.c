/**
 *****************************************************************************
   @example  ADCtimerDMA.c
   @brief    Demonstrate the use of the uDMA in flexi mode with ADC conversions triggered from timer overflow.
...
#define DMA_TRAN_COUNT 1024 // this is the maximum number of transfer for DMA
...
int main()
{
  ...
  AdcCnv(ADCCON_MOD_T0OVF,ADCCON_START); // start ADC conversions on timer overflow

  GptLd (pADI_TM0, 0x3000); 
  GptCfg(pADI_TM0, T0CON_CLK_UCLK, T0CON_PRE_DIV16, T0CON_ENABLE|T0CON_MOD_PERIODIC|TCON_RLD);

  /* this code places the part in FLEXI mode. the DMA interrupt wakes up the part */
  while (pADI_GP0->GPIN & 0x40)   {}  // wait for user to press IRQ2
  PwrCfg(PWRMOD_MOD_FLEXI,SCR_SLEEPONEXIT_DIS);
...
{
  AdcCnv(ADCCON_MOD_T0OVF,ADCCON_START_DIS); // stop ADC
